...
public class TuneableTrack {
    ...
    public int mode;
    /**
     * <p>The popularity of the track. The value will be between 0 and 100, with 100 being the most popular. The popularity is calculated by algorithm and is based, in the most part, on the total number of plays the track has had and how recent those plays are. <em><strong>Note</strong>: When applying track relinking via the <code>market</code> parameter, it is expected to find relinked tracks with popularities that do not match <code>min_*</code>, <code>max_*</code>and <code>target_*</code> popularities. These relinked tracks are accurate replacements for unplayable tracks with the expected popularity scores. Original, non-relinked tracks are available via the <code>linked_from</code> attribute of the <a href="https://developer.spotify.com/documentation/web-api/concepts/track-relinking">relinked track response</a>.</em></p>
     */
